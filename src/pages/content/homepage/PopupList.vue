<template>
  <div>
    ...
    <el-card style="margin-top: 20px;">
      ...
      <!-- 编辑界面 -->
      <!-- 弹窗 -->
      <el-dialog title="营销弹窗" :visible.sync="dialogFormVisible">
  ...
  export default {
    ...
    methods: {
      ...
      addPopup(newData) {
        ...
        requestPopupAdd(newData).then(res => {
          if (0 == res.id) {
      ...
      updatePopup(newData) {
        ...
        requestPopupUpdate(newData).then(data => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          requestPopupDelete(delData).then(data => {
