<template>
  <div>
    ...
    <el-card style="margin-top: 20px;">
      ...
      <!-- 编辑界面 -->
      <!-- 弹窗 -->
      <el-dialog title="启动页" :visible.sync="dialogFormVisible">
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let newData = {
              ...
              start_time: moment(this.editForm.date_range[0]).utcOffset(8).format('YYYY-MM-DD HH:mm:ss'),
              end_time: moment(this.editForm.date_range[1]).utcOffset(8).format('YYYY-MM-DD HH:mm:ss'),
            }
            if (this.editForm.index === -1) {
              this.addStartPage(newData)
            } else {
              newData.id = this.editForm.id
              this.updateStartPage(newData)
            }
          } else {
            console.log('error submit!!');
            return false;
      ...
      addStartPage(newData) {
        requestStartPageAdd(newData).then(res => {
          if (0 == res.id) {
      ...
      handleEdit(index, row) {
        this.editForm = {
          ...
          date_range: [new Date(row.start_time), new Date(row.end_time)]
        }

        this.dialogFormVisible = true
      },
      updateStartPage(newData) {
        requestStartPageUpdate(newData).then(data => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          requestStartPageDelete(delData).then(data => {
