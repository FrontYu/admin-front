    ...
    <vue-scroll>
      <el-menu :default-active="$route.path" class="sidebar-menu" :collapse="!openNav" @select="menuSelect"
        ...
        <template v-for="(level1, index1) in $router.options.routes">
          <template v-if="level1.menu && permissions.includes(level1.children[0].path)">
            ...
            <el-submenu :index="index1+''" v-if="level1.children.length > 1" :key="index1+''">
              <template slot="title"><i :class="level1.icon"></i><span
                  slot="title">{{$t(level1.name)}}</span></template>
              <template v-for="(level2, index2) in level1.children">
                <!-- 二级菜单 -->
                <!-- 无子菜单 start-->
                <el-menu-item v-if="!level2.children && permissions.includes(level2.path)" :index="level2.path"
                  :key="index1+'-'+index2">
                  {{$t(level2.name)}}
                </el-menu-item>
                <!-- 无子菜单 end-->
                ...
                <el-submenu :index="index1+'-'+index2" v-if="level2.children" :key="index1+'-'+index2">
                  <!-- 三级菜单 -->
                  <template slot="title"><i :class="level2.icon"></i><span
                      slot="title">{{$t(level2.name)}}</span></template>
